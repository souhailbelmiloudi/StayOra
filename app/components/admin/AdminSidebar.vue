<template>
  <aside
    class="fixed inset-y-0 start-0 z-30 flex flex-col border-e border-gray-200 bg-white transition-all duration-300"
    :class="collapsed ? 'w-16' : 'w-64'"
  >
    <div class="flex h-16 items-center justify-between border-b border-gray-100 px-4">
      <NuxtLink v-if="!collapsed" :to="localePath('/admin')" class="flex items-center gap-2">
        <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-xs font-bold text-white">
          S
        </div>
        <span class="text-lg font-bold tracking-tight">StayOra</span>
      </NuxtLink>
      <button
    ...
    <div class="border-t border-gray-100 p-3">
      <button
        class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 transition-all hover:bg-red-50 hover:text-red-600"
        @click="handleLogout"
      >
        <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
        </svg>
        <span v-if="!collapsed">{{ t('admin.nav.logout') }}</span>
      </button>
    </div>
  </aside>
</template>
...
function isActive(to: string): boolean {
  ...
  if (to === '/admin') return route.path === localized
